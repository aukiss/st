...
export async function handler(event){
  ...
  if(pass1.__safe_mode__){
      return { statusCode:200, body: JSON.stringify({ message:"运行于安全模式：未调用模型。请配置 OPENAI_BASE_URL 与 OPENAI_API_KEY 后再试。", safe_info: pass1 }) };
  }
  ...
  if(pass2.__safe_mode__){
      return { statusCode:200, body: JSON.stringify({ message:"运行于安全模式：未调用模型。请配置 OPENAI_BASE_URL 与 OPENAI_API_KEY 后再试。", safe_info: pass2 }) };
  }
